...
@section('content')
    <h3 class="text-3xl font-bold text-gray-800 mb-4">Tambah Karyawan</h3>
    <a href="/karyawan" class="inline-block text-gray-600 border-b border-gray-300 hover:text-blue-600">karyawan / <a
            href="{{ route('karyawan.create') }}" class="inline-block text-blue-600 border-b border-gray-300"> Create</a> </a>
                ...
                <select id="devisi_id" name="devisi_id"
                    ...
                    @foreach ($devisi as $devisiItem)
                        <option value="{{ $devisiItem->id }}" data-jam-mulai="{{ $devisiItem->jam_mulai }}"
                            ...
                            {{ old('devisi_id') == $devisiItem->id ? 'selected' : '' }}>
    ...
    {{-- js --}}
    <script>
        document.getElementById('devisi_id').addEventListener('change', function() {
